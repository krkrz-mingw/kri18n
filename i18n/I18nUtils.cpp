...
void I18nUtils::addMO(string filePath, string domain/*=DEFAULT_DOMAIN*/)
{
    if (!filePath.empty()) {
        domain = (!domain.empty() ? domain : DEFAULT_DOMAIN);
        _moMap[domain] = MO(filePath);
    }
}

void I18nUtils::addMO(unsigned char* bytes, string domain/*=DEFAULT_DOMAIN*/)
{
    if (bytes) {
        domain = (!domain.empty() ? domain : DEFAULT_DOMAIN);
        _moMap[domain] = MO(bytes);
    }
}
    ...
    for (size_t i = 0; i < matchItrsMap.size(); i++) {
        ...
        if (matchItr != matchItrsMap.end()) {
            ...
            switch (matchInfo.specifier) {
                case 'c':
                    if ("l" == matchInfo.type) {
						_snprintf_s(buf, MAX_LOG_LENGTH - 3, matchInfo.format.c_str(), (wchar_t)va_arg(args, int));
                    } else {
						_snprintf_s(buf, MAX_LOG_LENGTH - 3, matchInfo.format.c_str(), (char)va_arg(args, int));
                    }
                    break;
                    
                case 's':
                    if ("l" == matchInfo.type) {
						_snprintf_s(buf, MAX_LOG_LENGTH - 3, matchInfo.format.c_str(), va_arg(args, wchar_t*));
                    } else {
						_snprintf_s(buf, MAX_LOG_LENGTH - 3, matchInfo.format.c_str(), va_arg(args, char*));
                    }
                    break;
                ...
                case 'u':
                    if ("hh" == matchInfo.type) {
						_snprintf_s(buf, MAX_LOG_LENGTH - 3, matchInfo.format.c_str(), (unsigned char)va_arg(args, int));
                    } else if ("h" == matchInfo.type) {
						_snprintf_s(buf, MAX_LOG_LENGTH - 3, matchInfo.format.c_str(), (unsigned short)va_arg(args, int));
                    } else if ("l" == matchInfo.type) {
						_snprintf_s(buf, MAX_LOG_LENGTH - 3, matchInfo.format.c_str(), (unsigned long)va_arg(args, long));
                    } else if ("ll" == matchInfo.type) {
						_snprintf_s(buf, MAX_LOG_LENGTH - 3, matchInfo.format.c_str(), (unsigned long long)va_arg(args, long long));
                    } else {
						_snprintf_s(buf, MAX_LOG_LENGTH - 3, matchInfo.format.c_str(), (unsigned int)va_arg(args, int));
                    }
                    break;
                ...
                case 'F':
                    if ("L" == matchInfo.type) {
						_snprintf_s(buf, MAX_LOG_LENGTH - 3, matchInfo.format.c_str(), va_arg(args, long double));
                    } else {
						_snprintf_s(buf, MAX_LOG_LENGTH - 3, matchInfo.format.c_str(), va_arg(args, double));
                    }
                    break;
                ...
                case 'p':
					_snprintf_s(buf, MAX_LOG_LENGTH - 3, matchInfo.format.c_str(), va_arg(args, void*));
                    break;

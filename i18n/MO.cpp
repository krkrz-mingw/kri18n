...
bool MO::loadMoFile(std::string filePath)
{
    ...
    if (fin && fin.is_open()) {
        fin.seekg(0, fin.end);
        ssize_t size = static_cast<ssize_t>(fin.tellg());
        fin.seekg(0, fin.beg);
...
void MO::parseMoFile(unsigned char* bytes)
{
    ...
    for (unsigned long i = 0; i < moN; i++) {
        ...
        if (0 == originalLength && _headersInfo.empty()) {
			...
			// 提取 plural 参数
			EXTRACT_PLURAL_EXPRESSION(_headersInfo.c_str(), &plural, &nplurals);
			pluralExtracted = true;
        } else {
            /**
...
unsigned int MO::selectPluralForm(int count)
{
	if (pluralExtracted)
	{
		auto index = plural_eval(plural, count);
		if (index >= nplurals)
	...
	{
		// Return default English plural form.
		return (1 == count) ? 0 : 1;
	}
